...
export function DashboardMetricCard({
  ...
  return (
    <Card 
      className={`flex flex-col gap-1 rounded-lg p-3 border ${className}`}
      data-testid={testId}
    >
      <p className="text-xs font-medium leading-normal text-muted-foreground">
        {title}
      </p>
      <p className="tracking-tight text-xl font-bold leading-tight tabular-nums" data-testid={valueTestId}>
        {value}
      </p>
      {subtitle && (
